...
{|
 ...
 zdigits     := Int63Op.digits; (* number of digits *)
 to_Z        := Int63Op.to_Z; (* conversion to Z *)
 of_pos      := positive_to_int; (* positive -> N*int63 :  p => N,i
                                      where p = N*2^31+phi i *)
 head0       := Int63Op.head0;  (* number of head 0 *)
 tail0       := Int63Op.tail0;  (* number of tail 0 *)
 zero        := 0;
 one         := 1;
 minus_one   := Int63Op.max_int;
 compare     := Int63Op.compare;
 eq0         := Int63Op.is_zero;
 opp_c       := Int63Op.oppc;
 opp         := Int63Op.opp;
 opp_carry   := Int63Op.oppcarry;
 succ_c      := Int63Op.succc;
 add_c       := Int63Op.addc;
 add_carry_c := Int63Op.addcarryc;
 succ        := Int63Op.succ;
 add         := Int63Native.add;
 add_carry   := Int63Op.addcarry;
 pred_c      := Int63Op.predc;
 sub_c       := Int63Op.subc;
 sub_carry_c := Int63Op.subcarryc;
 pred        := Int63Op.pred;
 sub         := Int63Native.sub;
 sub_carry   := Int63Op.subcarry;
 mul_c       := mulc_WW;
 mul         := Int63Native.mul;
 ...
 gcd_gt      := Int63Op.gcd;
 gcd         := Int63Op.gcd;
 add_mul_div := Int63Op.addmuldiv;
 pos_mod     := pos_mod_int;
 is_even     := Int63Op.is_even;
 sqrt2       := Int63Op.sqrt2;
 sqrt        := Int63Op.sqrt;
 ZnZ.lor     := Int63Native.lor;
 ZnZ.land    := Int63Native.land;
 ZnZ.lxor    := Int63Native.lxor
|}.
...
Proof.
 ...
 assert (W:=to_Z_bounded p);assert (W':=to_Z_bounded Int63Op.digits);assert (W'' := to_Z_bounded w).
 case_eq (p <= Int63Op.digits)%int63;intros Heq.
 rewrite leb_spec in Heq.
 assert (0 <= [|p|] <= [|Int63Op.digits|]) by (auto with zarith).
 rewrite <- (shift_unshift_mod_3 [|Int63Op.digits|] [|p|] [|w|] H).
 replace ([|Int63Op.digits|] - [|p|]) with [|Int63Op.digits - p|];trivial.
 rewrite sub_spec, Zmod_small;auto with zarith.
(*
 symmetry;apply Zmod_small.
 rewrite <- Bool.not_true_iff_false, leb_spec in Heq.
 assert (2 ^ [|Int63Op.digits|] < 2 ^ [|p|]);[ apply Zpower_lt_monotone | ];auto with zarith.
 change wB with (2 ^ [|Int63Op.digits|]) in *;auto with zarith.
*)
Admitted.
...
Axiom land_spec : forall x y : int, [| Int63Native.land x y |] = Z.land [|x|] [|y|].
Axiom lor_spec : forall x y : int, [| Int63Native.lor x y |] = Z.lor [|x|] [|y|].
Axiom lxor_spec : forall x y : int, [| Int63Native.lxor x y |] = Z.lxor [|x|] [|y|].
