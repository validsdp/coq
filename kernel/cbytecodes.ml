...
type instruction =
  ...
  | Kprim of CPrimitives.operation * pconstant option
  | Kprim_const of CPrimitives.operation * pconstant option * int
  | Kareint of int
...
let rec pp_instr i =
  ...
  | Kprim (op, id) -> str (CPrimitives.operation_to_string op) ++ str " " ++
        (match id with Some (id,_u) -> Constant.print id | None -> str "")
  | Kprim_const (op, id, i) ->
      str (CPrimitives.operation_to_string op) ++ str "_const " ++
        int i ++ str " " ++
        (match id with Some (id,_u) -> Constant.print id | None -> str "")
